...
void CZ3View::CalcFig()
{
	...
	// Обработка граничных строк

	for (j = 1; j < ARSY-1; j++)	// левая вертикаль
	{
		i = 0;
	...
	for (j = 1; j < ARSY-1; j++)	// прввая вертикаль
	{
		i = ARSX-1;
		if (ARR[0][j - 1][i-1] == 1) SIGN++;
		if (ARR[0][j - 1][i] == 1) SIGN++;
	...
	for (i = 1; i < ARSX-1; i++)	// верхняя горизонталь
	{
		j = 0;
	...
	for (i = 1; i < ARSX-1; i++)	// нижняя горизонталь
	{
		j = ARSY-1;
		if (ARR[0][j-1][i - 1] == 1) SIGN++;
		if (ARR[0][j-1][i] == 1) SIGN++;
